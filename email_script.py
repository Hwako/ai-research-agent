...
def check_search_relevance(search_results: Dict[str, Any]) -> RelevanceCheckOutput:
    ...
    prompt_template = ChatPromptTemplate.from_messages([
        ("system", prompt),
        ("human", "Please review the search results above and return the relevant ones.")
    ])
    llm = ChatAnthropic(model="claude-sonnet-4-5").with_structured_output(RelevanceCheckOutput)
...
def generate_summaries(markdown_contents: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    summary_template = ChatPromptTemplate.from_messages([
        ...
        ("human", "Please summarize the page content above.")
    ])
...
def send_email(email_content: str):
    ...
    html_body = markdown.markdown(email_content, extensions=['extra'])
...
def main():
    ...
    output = graph.invoke({
        "messages": [HumanMessage(content="Please draft the email summary based on the research below.")],
        "summaries": summaries,
